        ...
        <div className="container mx-auto p-4">
            <div className="text-center">
                ...
                {editingProfile ? (
                    ...
                    <>
                        <h2 className="text-xl font-semibold mt-4">{manager.name || 'No Name'}</h2>
                        <p className="text-gray-600">{manager.email || 'No Email'}</p>
                        <button
                            className="mt-2 px-4 py-2 bg-blue-500 text-white rounded"
            ...
            {coaches.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                    {coaches.map(coach => (
                        <div
                            ...
                            onClick={() => navigateToCoachAppointments(coach.id)}
                        >
                            ...
                            <div className="flex justify-center mt-2">
                                <button
                                    className="mx-1 px-2 py-1 bg-yellow-500 text-white rounded"
                                    onClick={(e) => { e.stopPropagation(); navigate(`/edit-coach/${coach.id}`); }}
                                >
                                    Edit
                                </button>
                                <button
                ...
                <p className="mt-4 text-center text-gray-600">No Coaches</p>
            )}
        </div>
    );
};
