...
export default function ApprovalConfirmationDialog() {
  ...
  const handleApproveClick = () => {
    setIsOpen(false);
    // Here you could add logic to handle the Facebook sharing action
    // For now, just navigate to the video advertisements page
    navigate("/video-advertisements");
  };

  const handleCancelClick = () => {
    setIsOpen(false);
    // Navigate to the home page or another page on cancellation
    navigate("/video-advertisements");
  };
        ...
        <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-md mx-auto px-6 py-8">
          <div className="bg-white rounded-md shadow-lg px-6 py-8">
            ...
            <Dialog.Description className="mt-2 text-sm leading-relaxed text-center text-gray-500">
              Are you sure you want to share this content on Facebook too? This action will post the content to your Facebook account.
            </Dialog.Description>
            <div className="items-center gap-2 mt-4 text-sm sm:flex">
              <button
                onClick={handleApproveClick}
                className="w-full sm:w-auto mt-2 p-3 flex-1 text- bg-grsy-700 rounded-md outline-none ring-offset-2 bg-gray-500 focus:ring-2"
              >
                Yes, Share on Facebook
              </button>
              <button
