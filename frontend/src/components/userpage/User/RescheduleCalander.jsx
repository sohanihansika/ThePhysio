...
const Calender = () => {
  ...
  const handleDateChange = (selectedDate) => {
    ...
    if (isUnavailableDay(normalizedDate)) {
      ...
      navigate(`/timeslott?physioId=${physioId}&Slot=${CurrentSlot}&Id=${Id}&Olddate=${currentDate}&Newdate=${formattedNextDay}`);
  ...
  tileClassName={({ date, view }) => {
    if (view === 'month') {
      ...
      if (
        ...
        return 'bg-green-500 text-white'; // Add green styling
      }
